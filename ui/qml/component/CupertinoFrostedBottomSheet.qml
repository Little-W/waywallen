...
// Qcm's BottomSheet keeps its panel and gesture implementation private, so a
// transparent mdState color alone cannot blur the page content below it.  This
// component preserves the upstream sheet behaviour and adds a local, clipped
// Qt Quick blur behind the handle and content.
MD.Popup2 {
    id: control
    ...
    // `blurSource` must be the page content beneath this sheet, never the
    // popup parent, otherwise the panel can sample itself during animation.
    ...
    readonly property bool contentBlurEnabled: blurSource !== null
                                              && blurSource.width > 0
                                              && blurSource.height > 0
    readonly property int blurOverscan: Math.ceil(blurAmount * blurMax)
    readonly property rect blurSourceRect: {
        if (!blurSource)
            return Qt.rect(0, 0, 0, 0);
        const point = blurSource.mapFromItem(m_panel, 0, 0);
        return Qt.rect(point.x, point.y, m_panel.width, m_panel.height);
    }
    ...
    MD.Flickable2 {
        ...
        MD.ElevationRectangle {
            ...
            // ElevationRectangle only paints its shadow for a nonzero alpha.
            // The actual panel fill is provided by frostedBackground below.
            color: control.contentBlurEnabled
                ? Qt.rgba(control.surfaceColor.r, control.surfaceColor.g, control.surfaceColor.b, 0.001)
            ...
            Item {
                ...
                anchors.fill: parent
                enabled: false
                visible: control.contentBlurEnabled
                z: 0

                // The layer is clipped with the exact asymmetric sheet
                // corners, so sampled pixels never leak around the top edge.
                layer.enabled: true
                layer.effect: MD.RoundClip {
                    corners: control.mdState.corners
                    size: Qt.vector2d(frostedBackground.width, frostedBackground.height)
                }

                ShaderEffectSource {
                    id: sourceSnapshot
                    x: -control.blurOverscan
                    y: -control.blurOverscan
                    width: parent.width + control.blurOverscan * 2
                    height: parent.height + control.blurOverscan * 2
                    sourceItem: control.blurSource
                    sourceRect: Qt.rect(control.blurSourceRect.x - control.blurOverscan,
                                        control.blurSourceRect.y - control.blurOverscan,
                                        width,
                                        height)
                    live: control.visible && control.contentBlurEnabled
                    hideSource: false
                    recursive: false
                    visible: false
                    smooth: true
                ...
                MultiEffect {
                    ...
                    blurMax: control.blurMax
                }

                Rectangle {
                    anchors.fill: parent
                    color: Qt.rgba(control.surfaceColor.r,
                                   control.surfaceColor.g,
                                   control.surfaceColor.b,
                                   control.glassOpacity)
                }
            }
